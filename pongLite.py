...
class CVController:
    def __init__(self, paddle, cam, name):
        ...
        self.cam    = cam
        self.name   = name
        #TODO: Set the default hue/minContourArea to a value that works for your marker
        self.hue    = 123
        self.hueWidth   = 13
        self.minContourArea = 5
    ...
    def updatePos(self):
        ...
        img = cvData["img"]
        ##TODO:
        # process the image obtained from the camera to get an x and y position
        # and use it to control the paddle
        hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
        hue = self.hue
        hueWidth = self.hueWidth
        threshold = cv2.inRange(hsv,
        np.array((hue - (hueWidth/2), float(minBrightness), float(minBrightness))),
        np.array((hue + (hueWidth/2), 255.0, 255.0)))
        # fill new paddle canvas with zeros
        blobs_draw = np.zeros((height, width, 3), np.uint8)
        contour, hier = cv2.findContours(threshold, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
        i = 0
        for cnt in contour:
            # remove noise
            if (cv2.contourArea(cnt) < self.minContourArea):
                ...
                i += 1
                # find center of contour using boundingRect
        ...
        canvas.after(40, self.updatePos)
